...
def main(opt):
    ...
    for epoch in tqdm(range(start_epoch, args.epochs)):
        ...
        # training 
        train_loss=train(args, dataloading['train'], enc, dec, disc_l, disc_v,
                         optimizer_en, optimizer_de, optimizer_dl,optimizer_dv,                         
                         criterion_ae,criterion_ce,
                         Tensor, epoch, use_cuda)
        #validation
        valid_loss = valid(args, dataloading['valid'],enc,dec,disc_l, disc_v, epoch, use_cuda)
        # testing
        test_acc = test(args, dataloading['test'], enc, dec, disc_l, disc_v, epoch,use_cuda)
...
def train(args, trainloader, enc, dec, disc_l, disc_v,
          optimizer_en, optimizer_de, optimizer_dl, optimizer_dv,
...
def valid(args,validloader, enc, dec ,disc_l,disc_v, epoch, use_cuda):
    global best_acc
    batch_time = AverageMeter()
    data_time = AverageMeter()
    vloss = AverageMeter()
